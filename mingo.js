...
;(function (root) {
  ...
  Mingo.setup = function (options) {
    Object.assign(settings, options || {});return;
  };
  ...
  Mingo.addOperators = function (opClass, fn) {
    var newOperators = fn(Object.assign({
    ...
    assert([OP_AGGREGATE, OP_GROUP, OP_PIPELINE, OP_PROJECTION, OP_QUERY].includes(opClass), 'Could not identify operator class ' + opClass);
        ...
        each(newOperators, function (fn, op) {
          wrapped[op] = function (f, ctx) {
            return function () {
              ...
              var args = arraySlice.call(varargs);
  ...
  var pipelineOperators = {
    ...
    $project: function $project(collection, expr) {
      ...
      var _loop = function _loop(_i) {
        ...
        each(objKeys, function (key) {
          ...
          var objValue = void 0; // full object graph to value of the key
    ...
    $sort: function $sort(collection, sortKeys) {
      if (!isEmpty(sortKeys) && isObject(sortKeys)) {
        ...
        each(modifiers.reverse(), function (key) {
          ...
          var sortedIndex = {};
          var findIndex = function findIndex(k) {
            return sortedIndex[hashcode(k)];
          };
    ...
    $sample: function $sample(collection, expr) {
      var size = expr['size'];
      assert(isNumber(size), '$sample size must be a positive integer. See https://docs.mongodb.com/manual/reference/operator/aggregation/sample/');
    ...
    $count: function $count(collection, expr) {
      assert(isString(expr) && expr.trim() !== '' && expr.indexOf('.') === -1 && expr.trim()[0] !== '$', 'Invalid expression value for $count. See https://docs.mongodb.com/manual/reference/operator/aggregation/count/');
    ...
    $replaceRoot: function $replaceRoot(collection, expr) {
      ...
      each(collection, function (obj) {
        ...
        assert(isObject(obj), '$replaceRoot expression must return a valid JS object. ' + 'See https://docs.mongodb.com/manual/reference/operator/aggregation/replaceRoot/');
        result.push(obj);
      });
      return result;
    },
  ...
  var compoundOperators = {
    ...
    $and: function $and(selector, value) {
      ...
      each(value, function (expr) {
        return queries.push(new Mingo.Query(expr));
      });
    ...
    $not: function $not(selector, value) {
      var criteria = _defineProperty({}, selector, normalize(value));
  ...
  var groupOperators = {
    ...
    $avg: function $avg(collection, expr) {
      ...
      var sum = dataset.reduce(function (acc, n) {
        return acc + n;
      }, 0);
  ...
  var stringOperators = {
    ...
    $indexOfBytes: function $indexOfBytes(obj, expr) {
      ...
      assert(isUndefined(start) || isNumber(start) && start >= 0 && Math.round(start) === start, '$indexOfBytes third operand must resolve to a non-negative integer');
      start = start || 0;

      assert(isUndefined(end) || isNumber(end) && end >= 0 && Math.round(end) === end, '$indexOfBytes fourth operand must resolve to a non-negative integer');
      end = end || str.length;
    ...
    $dateToString: function $dateToString(obj, expr) {
      ...
      for (var i = 0, len = matches.length; i < len; i++) {
        ...
        if (isArray(hdlr)) {
          ...
          var fn = this[hdlr[0]];
          var pad = hdlr[1];
          _value = padDigits(fn.call(this, obj, date), pad);
        }
        // replace the match with resolved value
        fmt = fmt.replace(matches[i], _value);
      }
  ...
  each(['$eq', '$ne', '$gt', '$gte', '$lt', '$lte', '$in', '$nin'], function (op) {
    comparisonOperators[op] = function (obj, expr) {
      var _computeValue = computeValue(obj, expr, null),
          _computeValue2 = _slicedToArray(_computeValue, 2),
          lhs = _computeValue2[0],
          rhs = _computeValue2[1];
  ...
  var arrayOperators = {
    ...
    $range: function $range(obj, expr) {
      var _computeValue3 = computeValue(obj, expr, null),
          _computeValue4 = _slicedToArray(_computeValue3, 3),
          start = _computeValue4[0],
          end = _computeValue4[1],
          _computeValue4$ = _computeValue4[2],
          step = _computeValue4$ === undefined ? 1 : _computeValue4$;
    ...
    $zip: function $zip(obj, expr) {
      ...
      var len = 0;
      var arr = void 0; // temp variable
      var i = void 0; // loop counter
      ...
      for (i = 0; i < inputs.length; i++) {
        ...
        len = useLongestLength ? Math.max(len, arr.length) : Math.min(len || arr.length, arr.length);
      }
      ...
      for (i = 0; i < len; i++) {
        arr = inputs.map(function (val, index) {
          return isUnknown(val[i]) ? defaults[index] || null : val[i];
        });
        result.push(arr);
      }
  ...
  var aggregateOperators = Object.assign({}, arrayOperators, arithmeticOperators, booleanOperators, comparisonOperators, conditionalOperators, dateOperators, literalOperators, setOperators, stringOperators, variableOperators);
   ...
   */
  var redactVariables = {
    '$$KEEP': function $$KEEP(obj, expr, opt) {
      return obj;
    },
    '$$PRUNE': function $$PRUNE(obj, expr, opt) {
      return undefined;
    },
  ...
  function redactObj(obj, expr, opt) {
    ...
    return REDACT_VARS.includes(result) ? redactVariables[result](obj, expr, opt) : result;
  }
  ...
  function resolve(obj, selector, deepFlag) {
    ...
    var _loop3 = function _loop3(i) {
      ...
      if (isUndefined(value)) return 'break';
    };

    _loop4: for (var i = 0; i < names.length; i++) {
      var _ret3 = _loop3(i);
  ...
  function traverse(obj, selector, fn, force) {
    ...
    if (names.length === 1) {
      ...
      if (isArray(obj) && !isIndex) {
        each(obj, function (item) {
          return traverse(item, selector, fn, force);
        });
      } else {
  ...
  // quick reference for
  var primitives = [isString, isBoolean, isNumber, isDate, isUnknown, isRegExp];
  ...
  function isDate(v) {
    return isType(v, 'date');
  }
  function isRegExp(v, t) {
    return isType(v, 'regexp');
  }
  function isFunction(v, t) {
  ...
  function isEmpty(x) {
    return isUnknown(x) || isArray(x) && x.length === 0 || isObject(x) && keys(x).length === 0 || !x;
  }
  ...
  function flatten(xs, depth) {
    ...
    var arr = [];
    var unwrap = function unwrap(ys, iter) {
   ...
   * @return {Number}
   */
  function stddev(_ref) {
    var dataset = _ref.dataset,
        _ref$sampled = _ref.sampled,
        sampled = _ref$sampled === undefined ? false : _ref$sampled;

    var sum = dataset.reduce(function (acc, n) {
      return acc + n;
    }, 0);
    var N = dataset.length || 1;
    var err = sampled === true ? 1 : 0;
    var avg = sum / (N - err);
    return Math.sqrt(dataset.reduce(function (acc, n) {
